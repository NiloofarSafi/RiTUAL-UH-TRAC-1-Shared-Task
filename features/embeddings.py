...
class Word2VecFeatures(BaseEstimator, VectorizerMixin, TransformerMixin):
    ...
    def transform(self, documents):
        ...
        for i, doc in enumerate(documents):
            ...
            doc_feature_vecs[i], w, nw = self.make_feature_vec(analyze(doc.content))
            for w1 in w:
                if w1 not in wdic:
                    wdic.add(w1)
            for w2 in nw:
        ...
        print(len(wdic))
        print(len(wndic))
        print("the percentage of OOV is: ", len(wndic)/(len(wdic)+len(wndic)))
